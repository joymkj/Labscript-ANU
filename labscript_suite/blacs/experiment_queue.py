        ...
        while self.manager_running:
            ...
            try:  
                ...
                for line in all_lines:
                    line=line.strip()
                    if line:
                        if(line[0:13]=="MCS_stop_time"):
                            read_duration = floatfromstring(line)
                ...
                if(bin_size>0.1 and bin_size*10==int(bin_size*10) and read_duration>0 and plot_averages>0):            
                    sample_rate = 10     #KHz
                    no_of_samples = int(sample_rate*read_duration*1000)

                    with nidaqmx.Task() as task:
                        task.ci_channels.add_ci_count_edges_chan("ni_6363/ctr1")
                        task.timing.cfg_samp_clk_timing(sample_rate, source="pfi0", active_edge=Edge.RISING, sample_mode=AcquisitionType.FINITE, samps_per_chan=no_of_samples)
                        data = task.read(number_of_samples_per_channel=no_of_samples, timeout=read_duration+10)
                        save_data = "\n".join(map(str, data))

                        with open('C:/Users/sean/Documents/labscript/labscript_suite/logs/mcs_temp.txt','w+') as savefile:
                                savefile.write(save_data)
                        
                        if(plot_averages==1):
                            files = glob.glob('C:/Users/sean/Documents/labscript/labscript_suite/logs/mcs_avg/*')
                            for f in files:
                                os.remove(f)


                        elif(plot_averages>1):
                            random_filename = avg_path+str(uuid.uuid4())+'.txt'
                            os.makedirs(os.path.dirname(random_filename), exist_ok=True)
                            with open(random_filename,'w') as savefile:
                                savefile.write(save_data)

                            list_of_files = os.listdir(avg_path)
                            while len(list_of_files) > plot_averages:
                        ...
                        bin_array = cumulative2bin(data,int(bin_size*10))
            ...
            try:
                with h5py.File(path,'r+') as hdf5_file:
                    ...
                    h5_mcs_data = np.array(bin_array)

                    data_group = hdf5_file['/'].create_group('data')
                    if(bin_size>0.1 and bin_size*10==int(bin_size*10) and read_duration>0 and plot_averages>0):
                        dset = hdf5_file['/'].create_dataset("MCS data", dtype="i", data=h5_mcs_data)
                        dset.attrs['Stop Time (Seconds)'] = read_duration
                        dset.attrs['Bin size (millisecond)'] = bin_size
